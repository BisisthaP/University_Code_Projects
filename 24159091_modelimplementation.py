# -*- coding: utf-8 -*-
"""24159091_modelimplementation.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1IIbPb8-6S4TuLGpuy2H7jPWDef3C8-KD
"""

# Name - Bisistha Patra
# Student ID - 24159091

#This code is written and run on google colab - hence the file paths are different
#to run the code, use the files in the same zip folder as this .py file :)

#Code for part 1 - Naive Bayes from scratch
import pandas as pd
import numpy as np
...
#loading the training and testing dataset
    ...
    print("Initial missing values:", df.isna().sum())
...
def visualize_distribution(df):
    #this plot shows the distribution of ratings in the dataset - helping see class imbalance
...
def balance_dataset(df):
    #combining ratings 1 to 5 as one group and rest as individual groups for better visualization
...
class NaiveBayesScratch:
    ...
    def predict(self, X):
        ...
        # Initialising list to store scores for each class
        scores = []

        for idx in range(len(self.classes)):
            # Calculating the total weight of words in the review for this class [cite: 13]
            # Then adding it to the starting probability (prior) of the class [cite: 13]
            class_score = X.dot(log_likelihoods[idx, :]) + log_priors[idx]
            scores.append(class_score)

        # Converting to array and picking the highest score for prediction [cite: 26, 31]
...
#5. TESTING ON SAMPLE TEXT (UNSEEN DATA)
def test_custom_review(review_text, vectorizer, model):
    # Preprocessing the user input using the same function as training
    cleaned_input = preprocessing(review_text)

    # Transforming the text into numerical features
    # (Using the vectorizer fitted on training data)
    input_counts = vectorizer.transform([cleaned_input]).toarray()
...
def main():
    ...
    # To use this in your main block:
    sample_review = "The medication worked fast and I feel much better now, though I had a slight headache."
    test_custom_review(sample_review, vectorizer, nb_scratch)
